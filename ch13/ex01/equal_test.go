...
func TestCloseFloat(t *testing.T) {
	smallDiff := EPSILON / (1.0 + 1e-5)
	largeDiff := EPSILON * (1.0 + 1e-5)
	...
	for _, test := range []struct {
		...
		{0.0, 0.0, true},
		{0.0, 1.0, false},
		{1.0, 1.0 + smallDiff, true},
		{1.0, 1.0 - smallDiff, true},
		{1.0, 1.0 + largeDiff, false},
		{1.0, 1.0 - largeDiff, false},
		{
			complex(1.0, 1.0),
			complex(1.0-smallDiff, 1.0+smallDiff),
			true,
		},
		{
			complex(1.0, 1.0),
			complex(1.0-largeDiff, 1.0+largeDiff),
			false,
		},
	} {
